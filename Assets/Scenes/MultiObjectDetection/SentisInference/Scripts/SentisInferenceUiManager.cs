    ...
    public class SentisInferenceUiManager : MonoBehaviour
    {
        ...
        public void DrawUIBoxes(Tensor<float> output, Tensor<int> labelIDs, float imageWidth, float imageHeight)
        {
            ...
            for (var n = 0; n < maxBoxes; n++)
            {
                ...
                if (StaticClass.projectid != -1)
                {
                    var updated = (bool[])StaticClass.ObjectsFound.Clone();

                    for (var i = 0; i < updated.Length; i++)
                    {
                        if (m_labels[labelIDs[n]] == StaticClass.components[i].name)
                        {
                            updated[i] = true;
                        }
                    }
                    ...
                    StaticClass.ObjectsFound = updated; // triggers the event
                }
                else
                {
                 
                if (!DynamicProjectStaticClass.components.Contains(m_labels[labelIDs[n]]))
                {
                    var updatedList = new List<string>(DynamicProjectStaticClass.components);
                    updatedList.Add(m_labels[labelIDs[n]]);
                    DynamicProjectStaticClass.SetComponents(updatedList);
                }
